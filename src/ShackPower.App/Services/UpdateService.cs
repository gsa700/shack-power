...
/// <summary>
/// In-app updater, the family's standard: checks the GitHub latest release, downloads the build
/// for this platform, and (since a running executable can't overwrite itself) stages a helper
/// that waits for exit, swaps the exe, and relaunches. Cross-platform (win/linux/Pi).
/// </summary>
public static class UpdateService
{
    ...
    public const string Repo = "gsa700/shack-power";
    ...
    public static async Task<UpdateInfo> CheckAsync()
    {
        ...
        try
        {
            ...
            req.Headers.UserAgent.Add(new ProductInfoHeaderValue("ShackPower-UpdateCheck", "1.0"));
            ...
            var assetName = $"ShackPower-{Rid()}.zip";
    ...
    /// <summary>
    /// Temp directory the update is downloaded and unpacked into. The relaunched app must never
    /// have this as its working directory: a directory in use as one cannot be deleted, so the
    /// clean-up below would throw and updating twice without a restart would fail.
    /// </summary>
    private static string StageRoot => Path.Combine(Path.GetTempPath(), "ShackPower-update");
    ...
    public static async Task<string> DownloadAndStageAsync(string assetUrl)
    {
        ...
        {
            req.Headers.UserAgent.Add(new ProductInfoHeaderValue("ShackPower-UpdateInstall", "1.0"));
        ...
        var exeName = OperatingSystem.IsWindows() ? "ShackPower.exe" : "ShackPower";
    ...
    public static void ApplyAndRestart(string stagedExe)
    {
        ...
        {
            var ps1 = Path.Combine(Path.GetTempPath(), "shackpower-apply-update.ps1");
        ...
        {
            var sh = Path.Combine(Path.GetTempPath(), "shackpower-apply-update.sh");
    ...
    private static string UpdateFailedMarkerPath(string targetExe) =>
        Path.Combine(Path.GetDirectoryName(targetExe) ?? ".", ".shackpower-update-failed");
