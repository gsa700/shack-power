...
public class InstallLayoutTests
{
    ...
    {
        var downloads = Path.Combine("C:", "Users", "ab0r", "Downloads", "ShackPower-win-x64");
    ...
    {
        var stick = Path.Combine("E:", "ShackPower");
    ...
    public void TheHandUnzippedStationInstallIsAdoptedWhereItStands()
    {
        ...
        var legacy = Path.Combine(Programs, "ShackPower-win-x64");
        var mode = InstallLayout.Detect(legacy, portableMarkerPresent: false, InstallDir, Installed);
        Assert.Equal(InstallMode.Installed, mode);
    }

    [Theory]
    [InlineData("ShackPower")]
    [InlineData("ShackPower-win-x64")]
    [InlineData("ShackPower-linux-x64")]
    [InlineData("ShackPower-linux-arm64")]
    ...
    {
        var elsewhere = Path.Combine("D:", "tools", "shackpower");
    ...
    {
        Assert.Equal(Path.Combine(Programs, "Shack Power"),
    ...
    public void LinuxUsesTheXdgStyleFolderName()
    {
        ...
        Assert.Equal(Path.Combine(share, "shack-power"),
