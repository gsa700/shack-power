...
/// <summary>
/// Regression cover for the <c>~/.local/bin/shack-power</c> shortcut. The installer probed the link
...
public class SymlinkTests : IDisposable
{
    private readonly string _dir = Path.Combine(
        Path.GetTempPath(), "shackpower-symlink-tests", Guid.NewGuid().ToString("N"));
    ...
    public void ResolveTarget_returns_null_when_the_parent_directory_is_missing()
    {
        ...
        Assert.Null(Symlink.ResolveTarget(Path_("nested", "deep", "shack-power")));
    }
    ...
    {
        var target = Path_("app");
        var link = Path_("local", "bin", "shack-power");
        File.WriteAllText(target, "x");
